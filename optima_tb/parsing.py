...
class FunctionParser(object):
    ...
    def produceStack(self, string):
        ...
        if self.debug:
            ...
            print expr_stack
            print('Tokens that are variables...')
            print var_dict.keys()
    ...
    def evaluateStack(self, stack, deps = None, level = None):
        ...
        return self.parseStack(s, deps, level)
    ...
    def parseStack(self, stack, deps, level):
        """
        Parses the stack. Instead of popping elements from stack, an iterator is advanced

        :param stack: iterator of the stack (generated by produceStack)
        :param deps: dict of variable names and their values
        :param level: counter for recursion level
        :return: returns evaluated stack expression
        """
        op = stack.next()
        if op == 'u-':
            return -self.parseStack(stack, deps=deps, level=level + 1)
        elif op in "+-*/^":
            op2 = self.parseStack(stack, deps=deps, level=level + 1)
            op1 = self.parseStack(stack, deps=deps, level=level + 1)
            ...
            return self.ufn_dict[op](self.parseStack(stack, deps=deps, level=level + 1))
