...
def reconcile(proj, reconcile_for_year, parset, progset, sigma_dict=None, unitcost_sigma=0.05, attribute_sigma=0.20, budget_sigma=0.0, impact_pars=None, constrain_budget=True, budget_allocation=None, orig_tvec_end=None, max_time=None):
        """
        ...
        # Worry about extra metric at a later step...probably incorporate as an extra function?
        vals = dict() # This is a dict mapping par_uid:par_value for the reconciliation year
        for pop in results.model.pops:
            for par in pop.pars:
                if par.label in par_labels:
                    vals[par.uid] = par.vals[-1] # Final timepoint is at reconcile_for_year
        parset_vals = [vals[uid] for uid in par_uids] # Extract the list of parameter values from the parset

        def objective(pset,attribute_dict,par_uids,parset_vals,constrained_budget = None):
            # Load new attributes into pset, compute the parameter values, and then get the objective function

            # If budget is constrained
            if constrained_budget is not None: 
                rescaleAllocation(attribute_dict, constrained_budget) # Rescale the attribute dict in-place

            # Load the attributes into the pset
            ...
            # Note that the budget could change too
...
#             print('Unit Cost: %g' % attributes_dict[prog_label]['unit_cost'])
            attributes_dict[prog_label]['budget'] = progset.progs[index].getDefaultBudget(year=settings.tvec_end)
#             print('Budget: %g' % attributes_dict[prog_label]['budget'])
...
#                     print "key = ", key
                if key in ['cov' , 'dur' , 'time', 'cost']:
                    continue
#                 elif interpolated_attributes[key][-1] <= 0: # Note: AH had set this, before TB diagn had IPT component - which is often set to 0. Therefore ignoring this for the moment. SJ.
#                     mark_for_delete = True
#                     break
                else:
...
def updateProgset(pset, new_pars_dict, year):
...
#         print "new attr", new_attributes
#         print "attr dict", attribute_dict
#         print "\n======"*5
        proposed_dict = regenerateAttributesDict(new_attributes, attribute_dict)
#         print "\n!!!======"*5
