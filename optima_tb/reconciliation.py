...
from costcovfunction import CCFAttr
...
def reconcileFunc(proj, reconcile_for_year, parset_name='default', progset_name='default',
                  sigma_dict=None, unitcost_sigma=0.05, attribute_sigma=0.20, budget_sigma=0.0,
                  impact_pars=None, constrain_budget=True, budget_allocation=None, orig_tvec_end=None, max_time=None):
        """
        Reconciles progset to identified parset, the objective being to match the parameters as closely as possible with
        identified standard deviation sigma
...
def compareOutcomesFunc(proj, year, parset_name=None, progset_name=None, budget_allocation=None, compareoutcome=None, display=True, constrain_budget=True):
    ...
    if parset_name not in proj.parsets: raise OptimaException("ERROR: No parameter set '%s' found" % parset_name)
    if progset_name not in proj.progsets: raise OptimaException("ERROR: No program set '%s' found" % progset_name)

    # Set years for Simulation run
    ...
    if display:
        ...
        outcome = '\n\t\t\t%s\t%s\t%s\t%s\n' % (parset_value, progset_value_uncapped,
                                                progset_value_capped, progset_overflow)
        for par_label in impact.keys():
            if par_label != 'net_difference':
                outcome += '%s\n' % par_label
...
def createAttributeDict(settings, progset):
    ...
    # TODO change to refer to year that we're reconciling for, rather than the last year
    '''
    attributes_dict = odict()
    for prog_label in progset.prog_ids:

        mark_for_delete = False
        if prog_label not in attributes_dict:
            attributes_dict[prog_label] = odict()

        index = progset.prog_ids[prog_label]

        if np.isnan(progset.progs[index].getCoverage(0.1)):
            # it's a fixed program because the fixed program always return NaN
            mark_for_delete = True
        elif progset.progs[index].getDefaultBudget(year=settings.tvec_end) <= 0.:
            # program wasn't funded in the last year # TODO change to year that we're reconciling for
            mark_for_delete = True
        else:
            attributes_dict[prog_label]['unit_cost'] = progset.progs[index].ccf.attr[CCFAttr.uc]
            ...
            for key in interpolated_attributes:  # TODO generalise keys
        ...
        if prog_label not in new_pars_dict.keys(): continue
            ...
            for attribute in new_pars_dict[prog_label].keys():
                if attribute in progset.progs[index].attributes.keys():
                    ...
                    continue
            progset.progs[index].ccf.update({CCFAttr.uc: new_pars_dict[prog_label]['unit_cost']})
...
def reconciliationMetric(new_attributes, proj, parset, progset, parset_name, impact_pars, results, attribute_dict,
                         reconcile_for_year, compareoutcome, prog_budget_alloc, constrain_budget):
    ...
    if not compareoutcome:
        ...
        if constrain_budget:
            print('Total Budget: %g\tProposed Budget: %g\tConstrained Budget: %g\n' % (orig_total_budget,
                                                                                       ...
                                                                                       constrained_total_budget))
            for key in attribute_dict.keys():
                print('Program: %s\n  Original Unit Cost: %g\tProposed Unit Cost: %g\tConstrained Unit Cost: %g' %
                      (key, attribute_dict[key]['unit_cost'], proposed_dict[key]['unit_cost'],
                       constrained_dict[key]['unit_cost']))
    ...
    logging.info("Reconciliation metric: ")
    par_attributes = odict()
    prog_attributes = odict()
    parser = FunctionParser(debug=False)

    for par_label in impact_pars:
        for popkey in results.pop_label_index:
            if popkey not in prog_attributes:
                prog_attributes[popkey] = odict()
    ...
    for par_label in (impact_pars):
        for popkey in results.pop_label_index.keys():
            ...
            for index in range((len(parset))):
                ...
                elif (par_label == parset[index].label) and (par_label in proj.settings.linkpar_specs):
                    if par_label not in par_attributes[popkey].keys(): par_attributes[popkey][par_label] = odict()
                    if 'f_stack' in proj.settings.linkpar_specs[par_label]:
