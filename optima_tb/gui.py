...
#        do_options_exist = self.options is not None
    ...
    def reconcile(self):
        ...
        self.refreshStatus()
        if True: # try:
            self.progset, output = self.project.reconcile(parset_name=self.parset_name, progset=self.progset,
                                                          reconcile_for_year=self.options['progs_start'],
                                                          sigma_dict=self.sigma_dict, overwrite=True, # constrain_budget=False,
                                                          max_time=reconciliation_time, save_progset=False)
